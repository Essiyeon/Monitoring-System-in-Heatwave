...
# MariaDB 연결 설정
conn = pymysql.connect(host="localhost", user="raspi_user", passwd="dsp502", db="raspi_db")
...
def index():
    ...
    return render_template('index01.html', temperature=temperature, humidity=humidity, feel_like_temp=feel_like_temp)
        ...
        try:
            ...
            if humidity is not None and temperature is not None:
                ...
                with conn.cursor() as cur:
                    sql = "INSERT INTO workplace_data VALUES (%s, %s, %s, %s, %s)"
                    cur.execute(sql, ('DHT22', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
                                      temperature, humidity, feel_like_temp))
                    conn.commit()
